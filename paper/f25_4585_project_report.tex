...
At the same time, the limitations of our model are significant. We attack only the final round, not the full 16-round cipher; we assume ideal chosen-plaintext access to the encryption oracle; and we do not attempt to propagate differentials across multiple rounds or invert the full key schedule to recover the 56-bit master key. In practice, modern attackers would simply brute force single-DES or target Triple DES and legacy protocols, while contemporary standards have moved to AES and other ciphers with larger keys and stronger security margins. Our contribution is therefore best understood as an educational case study rather than a new break.
