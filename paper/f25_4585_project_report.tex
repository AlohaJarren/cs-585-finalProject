...
%------------------------------------------------------------------------------%
    ...
    The attack we have implemented is based on ``Differential Cryptanalysis of DES-like Cryptosystem'' by Eli Biham and Adi Shamir~\cite{10.5555/646755.705229}. In this article, Biham and Shamir give an overview of DES and its vulnerabilities, introduce differential cryptanalysis, and show how it is possible to recover the key of reduced-round DES with relative efficiency.
    

\subsubsection*{Organization}

We will first go over how the DES encryption scheme works after defining relevant terms. We will then go into how our attack on single-round DES was implemented, and how this attack can be applied to other forms of DES and DES-like schemes.
...
\begin{description}[style=unboxed, leftmargin=2em, font=\itshape]
    \item[Numbers:] To differentiate between hexadecimal, decimal, and binary numbers a subscript is used to denote the number's base. Decimal numbers are denoted with no subscript. (e.g., $31 =\text{1F}_{16} = 00011111_{2}$)
    ...
    \item[Feistel Rounds:] The state of the ciphertext after round $i$ is denoted by the pair $(L_i, R_i)$, where $L_i$ denotes the leftmost 32-bits and $R_i$ denotes the rightmost 32-bits. The pair $(L_0, R_0)$ is used to denote the input to the first round of the feistel network.
    ...
    \item[Subkeys:] The subkeys are denoted by $K_i$, where $i$ indicates the round in which the subkey is used. The notation $K^{(j)}$ is used to denote the $j$-th 6-bit chunk of $K$.
...
inputting a single 64-bit key into the key generation algorithm. Note that the true key space of DES is 56 bits. When a 64-bit key is given as input, the 8th bit of each byte is not used.
    The 56 bits that are used are then passed through a permutation. This permutation is the same
for all keys. The left half of the output of this permutation becomes $C_0$ and the right half becomes $D_0$. We then use a series of left shifts on $C_0$ and $D_0$ to obtain $C_n$ and $D_n$ for $1 \le n \le 16$. for $n = 1, 2, 9, 16$ $C_n$ and $D_n$ are a single left shift from $C_{n-1}$ and $D_{n-1}$, and for the other values of n, two left shifts are used. We then have sixteen 56-bit strings $C_nD_n$ for $1\le n\le16$. To obtain our 48-bit subkeys $K_n$ we apply a permutation to 48 bits of the corresponding $C_nD_n$. For reduced round DES of n rounds, we can just use the first n keys.

%------------------------------------------------------------------------------%
\subsection{Encryption Overview}
%------------------------------------------------------------------------------%

To encrypt a 64 bit message under DES, the message is first passed through a fixed initial permutation $IP$ that rearranges the bits. This permutation is the same for every encryption. The output of this permutation becomes the input to the first round. The output of that round becomes the input for the second round. This continues until the output of the sixteenth round, which is run through the inverse of $IP$. The output of that inverse permutation is the ciphertext.
...
The goal of our attack is to generate a number ciphertext pairs $(C, C')$ where $\Delta B$ and $\Delta Y$ are known to the adversary. Using such a ciphertext pair, an adversary can use $L_1$ to compute a subset of possible subkeys able to produce this result. Given enough of these ciphertext pairs, an adversary can reduce the subkey space considerably, and it becomes possible to brute-force the exact subkey. The general algorithm is as follows:
\begin{enumerate}
    ...
    \item Encrypt all pairs in $\mathbf{M}$ to get a set of ciphertext pairs $\mathbf{C}$.
...
This attack does not suffer any probability of failure, i.e., the true subkey $K$ is not in $\mathbf{K}$, however the reduction in key space does have significant variance between trials. Using the candidates we selected, we found that the possible 6-bit subkey fragments was ceased shrinking at only 2 candidates for all S-box positions except $S^2$ and $S^4$ which produced $4$ candidates each. This creates a total key space of only $2^{10}$ which is trivial to brute force on a laptop on the order of milliseconds.
...
%------------------------------------------------------------------------------%
\subsection{Translating to Multi-Round DES}
\label{sec:impact}
%------------------------------------------------------------------------------%

The purpose of our attack model is to provide insight on differential cryptanalysis on multi-round feistel networks. One potential way our work can be applied to multi-round attacks is in the construction and testing of characteristics. In an attack against an $n$-round scheme, the characteristic we choose is instead a series of differentials $(L_0^*, R_0^*), (L_1^*,R_1^*), \dots, (L_n^*,R_n^*)$, where in each step $L_i^*, R_i^*$ and $L_{i-1}^*, R_{i-1}^*$ are akin to $M^*$ and $C^*$ in our 1-round model. It logically follows that the best possible characteristic for multi-round attacks is comprised of a chain of ideal 1-round characteristics and ends with the best possible 1-round characteristic.

It is here we must admit the critical differences between our 1-round attack model, and a single round in a multi-round scheme. Firstly, in a multi-round DES scheme it is not necessary or possible to filter $\mathbf{C}$ to exclusively be ciphertexts where $\Delta Y = Y^*$ in the final round. This is because we cannot control $\Delta L_{n-1}$ to always be $0$. Also, we would not be able to conclude that $\Delta R_{n-1} = R_{n-1}^*$ with absolute certainty. Instead, it is assumed that $\Delta B = B^*$ and $\Delta Y = Y^*$ is true with a higher probability than expected from an ideal encryption algorithm. Instead, the entire $\mathbf{C}$ is used in the key voting step and the true subkey while show with high probability.

This brings an interesting component to our attack, in that the filtering step to $\mathbf{C}^*$ is actually necessitated in 1-round DES, when it is unneeded in multi-round attacks. This is because in multi-round DES it can be assumed that if $\Delta Y \ne Y^*$ the subkeys produced by the key voting algorithm will be approximately uniformly distributed. However, because less entropy exists in only a single round of DES, this assumption no longer holds if the entirety of $\mathbf{C}$ is used all subkeys show up with a roughly uniform distribution
...
There are a few major limitations to the attack we have presented. The most important of which is the naive selection of characteristics. This attack method is almost entirely dependent on the quality of the chosen characteristic $(B^*, Y^*)$. This is mostly because our search method only considers 3 viable input XOR values for each S-box. It is possible that a characteristic with a lower differential probability could still provide a more efficient attack by revealing subkey bits from multiple S-boxes at a time.

Another potential improvement on our attack would minimize the presence of key ties in our key reduction step. This would improve the fidelity of the key reduction step and lower the key space reduction further. This could be done by searching characteristics with lower probabilities of occurrence, or by using multiple characteristics and finding the overlap between their possible subkeys. Either would increase the number of encryptions needed to perform the attack, but reduce the time to brute force the remaining key space. This is our recommendation for future work, because the number of encryptions required to accomplish this attack is already sufficiently small.
...
This project implemented and experimentally evaluated a differential cryptanalysis attack on a one-round instance of DES, with the goal of recovering the 48-bit last-round subkey. By constructing S-box difference distribution tables, generating plaintext pairs with a fixed input XOR, and letting each resulting ciphertext pair vote on candidate subkeys, our implementation reduced the last-round key space from $2^{48}$ possible values to roughly $2^{10}$-$2^{12}$ candidates. In our experiments, about $1600$ one-round encryptions across all eight S-boxes were sufficient to reach that range, after which an exhaustive search over the remaining candidates identified the correct subkey.

Our attack model was intentionally constrained to resemble an adversary that can effectively isolate a single round inside a full DES instance. In particular, the differential step is allowed to use only ciphertexts and the fact that the unknown plaintexts satisfy a chosen input XOR, making the model closer to a known-plaintext differential attack than to the stronger chosen-plaintext setting usually assumed in the classical analysis. This is consistent with the ``ciphertext pair'' viewpoint of Biham and Shamir, where the cryptanalyst does not need to know the actual plaintext values as long as their difference is controlled~\cite{10.5555/151116,10.5555/646755.705229}.

Targeting the final round key is also not a weakness of the model, but standard practice in differential cryptanalysis. Classical attacks on DES and DES-like ciphers first recover the last-round subkey and then work backwards, peeling off rounds and inverting the key schedule~\cite{10.5555/151116,10.5555/646755.705229}. In DES, learning a 48-bit round key already reveals most of the 56-bit master key; the remaining bits can be searched with negligible extra effort compared to the overall attack. Our one-round experiment can therefore be interpreted as isolating precisely this last-round phase and measuring how quickly differential biases concentrate probability mass on a small set of key candidates.

When compared with the original work of Biham and Shamir, our results are qualitatively compatible, but quantitatively much smaller in scale. Their full 16-round differential attack on DES requires about $2^{47}$ chosen plaintexts to obtain roughly $2^{36}$ useful ciphertext pairs and then performs on the order of $2^{37}$ DES operations in the data analysis phase~\cite{10.5555/151116,10.5555/646755.705229}. In contrast, our setting involves only a single round and a deliberately simplified model, so it is not surprising that we observe meaningful key-space reductions after roughly $2^{11}$ one-round encryptions. The important point is that the same statistical mechanism is visible: biased differentials allow us to rule out the vast majority of candidate subkeys and leave a tiny subset for brute force.

Overall, the project confirms that differential cryptanalysis is not just a theoretical construct, but a practical method for recovering key material, even when the attack is restricted to ciphertexts and a relatively weak notion of control over the plaintexts. At the same time, the dramatic gap between our $2^{10}$-sized candidate set in $\approx 1600$ encryptions and the $2^{47}$-plaintext, $2^{37}$-time regime required for full 16-round DES highlights why differential attacks on standard DES remain primarily of academic interest rather than a real-world threat~\cite{Heys01072002}. Future work, as outlined in our discussion section, would involve extending the analysis to multiple rounds, chaining characteristics across the Feistel structure, and explicitly mapping recovered last-round subkeys back to the 56-bit master key to more closely mirror the full Biham-Shamir attack.
