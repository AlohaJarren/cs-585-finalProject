...
def permute(data: int, bits: int, table: Tuple[int]) -> int:
    ...
    :param data: The input you wish to permute
    :param bits: The number of bits in data
    :param table: The permutation table used
    :ret: The permuted data
    """
    ret = 0
...
def IP(block: int, invert: bool = False) -> int:
    if invert:
        return permute(block, 48, INITIAL_INV)
    return permute(block, 32, INITIAL)

def E(block: int, invert: bool = False) -> int:
    """
    Implements the 32-bit to 48-bit mapping of the Expansion Box

    :param block: A 32-bit input to the Ebox
    :ret: A 48-bit output from the Ebox
    """
    if invert:
        return permute(block, 48, EXPANSION_INV)
    return permute(block, 32, EXPANSION)

def P(block: int, invert: bool = False) -> int:
    """
    Implements the 32-bit to 32-bit mapping of the Permutation Box

    :param block: A 32-bit input to the Pbox
    :ret: A 32-bit output from the Pbox
    """
    if invert:
        return permute(block, 32, PERMUTATION_INV)
    return permute(block, 32, PERMUTATION)
